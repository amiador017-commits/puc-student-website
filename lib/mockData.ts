...
// ═══════════════════════════════════════════════════════════════
// Real PUC CSE Course List (U-Version-4)
// ═══════════════════════════════════════════════════════════════
...
export function calculateGPA(courses: Course[]): number {
  ...
  for (const c of courses) {
    const totalMarks = c.grades.midterm + c.grades.final;
    const maxMarks = c.grades.midtermMax + c.grades.finalMax;
    if (maxMarks === 0) continue; // skip courses with no max marks set
    const percentage = (totalMarks / maxMarks) * 100;
    let gpa = 0;
    if (percentage >= 90) gpa = 4.0;
    else if (percentage >= 85) gpa = 3.75;
    else if (percentage >= 80) gpa = 3.5;
    else if (percentage >= 75) gpa = 3.25;
    else if (percentage >= 70) gpa = 3.0;
    else if (percentage >= 65) gpa = 2.75;
    else if (percentage >= 60) gpa = 2.5;
    else if (percentage >= 55) gpa = 2.25;
...
export const SCHEDULE: ScheduleEntry[] = [
  // ── Saturday ──
  {
    ...
    duration: "1h 15min",
    courseCode: "CSE 1110",
    courseName: "Competitive Programming Lab",
    room: "404",
    instructor: "DAB",
    color: "neon",
  },
  {
    ...
    duration: "1h 15min",
    courseCode: "EEE 1201",
    courseName: "Electronics Devices & Circuits",
    room: "1002",
    instructor: "AIR",
    color: "rose",
  },
  ...
  },
  {
    ...
    duration: "1h 15min",
    courseCode: "CSE 1411",
    courseName: "Discrete Math & Number Theory",
    room: "610",
    instructor: "FSC",
    color: "violet",
  },
